...
public:
    ...
    Base(Field *obj, log::Adapter* adapter) : field(obj), adapter(adapter){
            health = 2000;
    ...
    vector<Item *> get_units() {
        return units->getUnits();
    }

    unsigned int get_max_item(){
        return max_item;
    }

    unsigned int get_now_item(){
        return now_item;
    }
    ...
    void set_char(int health, int n_a, int f_a){}
    ...
    void add_green_dr(unsigned int x, unsigned int y, string &logs){
        if (now_item < max_item){
            ...
            throw Limit_Exception("Maximum number of units!", max_item);
